  ...
  export default {
    ...
    methods: {
      async loadRooms() {
        try {
          const { data } = await axios.get('https://backend-qlks.onrender.com/room');
          this.rooms = data.map(r => ({
            roomid: r.RoomID,
            roomtype: r.RoomType,
            price: r.Price,
            status: r.Status === 'Available' ? 'Trống' : 'Đã đặt',
            image: r.image ? `https://backend-qlks.onrender.com/images/${r.image}` : 'https://via.placeholder.com/200'
          }));
        } catch (err) {
      ...
      async confirmBooking() {
        try {
          ...
          await axios.post('https://backend-qlks.onrender.com/reservation', payload);
      ...
      async cancelBooking(room) {
        try {
          const response = await axios.put(`https://backend-qlks.onrender.com/room/${room.roomid}/cancel`);
  ...
  try {
    ...
    const response = await axios.get(`https://backend-qlks.onrender.com/reservation/${room.roomid}`);
  ...
  try {
    ...
    await axios.post('https://backend-qlks.onrender.com/payment', payload);
      ...
      async confirmEdit() {
        try {
          const payload = {
              ...
              await axios.put(`https://backend-qlks.onrender.com/room/${this.selectedRoom.roomid}`, {
                Status: 'Occupied'  // Hoặc sử dụng giá trị trạng thái phù hợp
              });

          await axios.put(`https://backend-qlks.onrender.com/room/${this.editRoom.roomid}`, payload);
